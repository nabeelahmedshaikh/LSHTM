...
TB_model_3 <- function(pars, state, times = seq(0, tend)) {
  derivs <- function(t, state, pars) {
    with(as.list(c(state, pars)), {
      ...
      # detection rate - allows it to be changed to alter the shape of the incidence curve
      cov <- approxfun(x = c(1, 2), y = c(0, cov_PT), method = "linear", rule = 2)  
      
      # Check total population stays at 1
      Total <- S + L + I + P + T
      
      # Births set to total deaths to keep the total population constant 
      births <- u * Total + m * I 
      ...
      dI <- FOI * S * a +                      # primary disease
        ...
        tau * I -                             # minus recovery
        u * I -                               # minus death
        r * I * T                             # minus outflow to T
      ...
      # Return these things
      return(list(c(dS, dL, dI, dP, dT, dN_PT),
                  Total = Total,
                  Inc = Inc,
                  Number_PT = Number_PT))
    })
